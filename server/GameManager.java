...
public class GameManager extends Thread {
    ...
    public GameManager() {
        ...
        tcpServer = new TCPServer(Constants.serverPort,this);
    ...
    public synchronized void updatePlayers(Map<Socket, String> commands) {
        ...
        commands.keySet().forEach((socket) -> {
            ...
            if (command[0].equals("$") && command[command.length - 1].equals("$")) {
                if (command[1].equals("verify")) {
                    ...
                    if (dataBase.checkCredentials(command[2], command[3]) && isOK) {
    ...
    public void newPlayer(String name, String password, Socket socket) {
        ...
        if (!players.contains(newPlayer)) {
            ...
            tableManager.getTable(tableid).getGame().wakeup();
